...
def scanForSecrets(yaml_d): 
    ...
    '''    
    logging.info("Scan for secrets initiated at {}".format(datetime.now()))
    for key_data  in key_lis:
        key_     = key_data[0]
        value_list = [] 
        parser.getValsFromKey( yaml_d, key_ , value_list )
        logging.info("Scanning for hard-coded usernames.")
        unameList = scanUserName( key_, value_list  )
        # print(unameList)
        logging.info("Scanning for hard-coded passwords.")
        passwList = scanPasswords( key_, value_list  )
        logging.info("Scanning for hard-coded keys.")
        ...
        if( len(unameList) > 0  )  or ( len(passwList) > 0  ) or ( len(keyList) > 0  ) :
            ...
            logging.info("Hard-coded secrets found!")
...
def scanForOverPrivileges(script_path):
    ...
    if(checkVal): 
        logging.info("Valid Kubernetes YAML detected: {}".format(script_path)) # Log (2)
        dict_as_list = parser.loadMultiYAML( script_path )
        yaml_dict    = parser.getSingleDict4MultiDocs( dict_as_list )
        logging.info("YAML converted to a single dictionary.") # Log (3)
        # print(yaml_dict.keys())
        key_lis   = []
        parser.getKeyRecursively(yaml_dict, key_lis)
        logging.info("Keys extracted: {}".format(key_lis)) # Log (4)
        # print('KEY LIST ALL-------------------------------------')
        # print(key_lis)
        '''
        if you are using `parser.getKeyRecursively` to get all keys , you need to do some trnasformation to get the key names 
        ...
        logging.info("Processed keys: {}".format(just_keys)) # Log (5)
        # print('JUST KEYS ALL -----------------------------------------------------')
        # print(just_keys)
        ...
        if ( constants.KIND_KEY_NAME in just_keys ):
            ...
            logging.info("Kind values found: {}".format(kind_values)) # Log (6)
        '''
        ...
        if ( constants.PRIVI_KW in just_keys ) and ( constants.DEAMON_KW not in kind_values  ) :
            ...
            logging.info("Privilege values found: {}".format(privilege_values)) # Log (7)
            # print(privilege_values) 
            for value_ in privilege_values:
                    if value_ == True: 
                        key_lis_holder = parser.keyMiner(yaml_dict, value_ ) 
                        logging.info("Over-privileged context detected.") # Log (8)
                        ...
                        if(constants.CONTAINER_KW in key_lis_holder) and (constants.SECU_CONT_KW in key_lis_holder) and (constants.PRIVI_KW in key_lis_holder):
                            ...
                            logging.info("Line numbers for privileged context occurrences: {}".format(line_number)) # Log (9)
                            for line in line_number:
                                ...
                                logging.info("SARIF result added for line: {}".format(line)) # Log (10)
    logging.info("Scan for over-privileged security contexts completed at {}".format(datetime.now())) # Log (11)
...
def scanForDefaultNamespace(path_scrpt):
    ...
    dic, lis   = {}, []
    if ( parser.checkIfValidK8SYaml( path_scrpt )  ): 
        logging.info("Valid Kubernetes YAML detected: {}".format(path_scrpt)) # Log (2)
        cnt = 0 
        dict_as_list = parser.loadMultiYAML( path_scrpt )
        logging.info("Loaded YAML as a list of dictionaries.") # Log (3)
        yaml_di      = parser.getSingleDict4MultiDocs( dict_as_list )        
        logging.info("Converted YAML list to a single dictionary.") # Log (4)
        nspace_vals  = []
        parser.getValsFromKey( yaml_di, constants.NAMESPACE_KW, nspace_vals )
        logging.info("Namespaces extracted: {}".format(nspace_vals)) # Log (5)
        '''
        we are not going to process list of dicts 
        '''
        nspace_vals        = [x_ for x_ in nspace_vals if isinstance( x_ , str ) ]
        logging.info("Filtered namespace values to include only strings: {}".format(nspace_vals)) # Log (6)
        unique_nspace_vals =  list( np.unique( nspace_vals  ) )
        logging.info("Unique namespace values: {}".format(unique_nspace_vals)) # Log (7)
        if (len(unique_nspace_vals) == 1 ) and ( unique_nspace_vals[0] == constants.DEFAULT_KW  ): 
            logging.warning("Default namespace detected in file: {}".format(path_scrpt)) # Log (8)
            key_lis = parser.keyMiner(yaml_di, constants.DEFAULT_KW)
            logging.info("Keys associated with the default namespace: {}".format(key_lis))  # Log (9)

            if (isinstance( key_lis, list ) ):
                if (len(key_lis) > 0 ) : 
                    all_values = list( parser.getValuesRecursively(yaml_di)  )
                    logging.info("All values extracted from the YAML: {}".format(all_values))  # Log (10)
                    cnt += 1 
                    
                    # Lines for SARIF output
                    line_number = parser.show_line_for_paths(path_scrpt,constants.NAMESPACE_KW)
                    logging.info("Line numbers for default namespace occurrences: {}".format(line_number))  # Log (11)
                    for line in line_number:
                        ...
                        result.locations = [location]
                        run.results.append(result)
                        logging.info("SARIF result added for line: {}".format(line))  # Log (12)
                    prop_value = constants.YAML_SKIPPING_TEXT 
                    if ( constants.DEPLOYMENT_KW in all_values ) : 
                        prop_value = constants.DEPLOYMENT_KW
                        lis.append( prop_value )
                        logging.info("Deployment keyword found and added to the list.")  # Log (13)
                    elif ( constants.POD_KW in all_values ) :
                        prop_value = constants.POD_KW 
                        lis.append( prop_value )
                        logging.info("Pod keyword found and added to the list.")  # Log (14)
                    else: 
                        holder_ = [] 
                        parser.getValsFromKey(yaml_di, constants.KIND_KEY_NAME, holder_ )
                        logging.info("Kind values extracted: {}".format(holder_))  # Log (15)
                        if ( constants.K8S_SERVICE_KW in holder_ ): 
                            srv_val_li_ = [] 
                            parser.getValsFromKey( yaml_di, constants.K8S_APP_KW, srv_val_li_  ) 
                            logging.info("Service values extracted: {}".format(srv_val_li_))  # Log (16)
                            for srv_val in srv_val_li_:
                                lis = graphtaint.mineServiceGraph( path_scrpt, yaml_di, srv_val )
                                logging.info("Service graph mined for value: {}".format(srv_val))  # Log (17)


                    dic[ cnt ] = lis
                    logging.info("Final dictionary entry for default namespace: {}".format(dic[cnt]))  # Log (18)
    # print(dic) 
...
def scanForResourceLimits(path_scrpt):
    ...
    if ( parser.checkIfValidK8SYaml( path_scrpt )  ): 
        ...
        logging.info("Keys extracted: {}".format(temp_ls))  # Log (5)
        '''
        if you are using `parser.getKeyRecursively` to get all keys , you need to do some trnasformation to get the key names 
        as the output is a list of tuples so, `[(k1, v1), (k2, v2), (k3, v3)]`
        '''
        key_list = [ x_[0] for x_ in temp_ls  ]
        logging.info("Processed keys: {}".format(key_list))  # Log (6)
        '''
        ...
        logging.info("Kind entries found: {}".format(kind_entries))  # Log (7)
        if ( constants.POD_KW in kind_entries ):
            if ( (constants.CONTAINER_KW in key_list) and (constants.LIMITS_KW not in key_list ) and ( (constants.CPU_KW not in key_list)  or (constants.MEMORY_KW not in key_list) ) ):
                logging.warning("Missing resource limits detected in file: {}".format(path_scrpt))  # Log (8)
                cnt += 1
                # the line number for SARIF output should be the line number of the container spec 
                line_number = parser.show_line_for_paths(path_scrpt, constants.SPEC_KW)
                logging.info("Line numbers for missing resource limits: {}".format(line_number))  # Log (9)
                for line in line_number:
                ...
                if( len(temp_ls) > 0 ):
                    ...
                    if ( constants.POD_KW in all_values ) :
                        prop_value = constants.POD_KW 
                        lis.append( prop_value )
                        logging.info("Pod keyword found and added to the list.")  # Log (12)
                dic[ cnt ] = lis
    # print(dic) 
    logging.info("Scan for resource limits completed at {}".format(datetime.now()))  # Log (13)
    return dic 
